...
// ---------- Cal wrapper — remounts on key change ----------
function CalEmbed({ calLink }: { calLink: string }) {
  const [ready, setReady] = useState(false);
  // give the iframe an extra tick after getCalApi resolves
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      // short delay lets the iframe paint at full width before we reveal it
  ...
  return (
    // outer div always reserves full width + min height → no layout shift
    <div className="relative w-full min-h-[650px]">
      {/* skeleton sits behind until ready */}
      {!ready && (
      ...
      {/* Cal iframe always rendered but invisible until ready */}
      <Cal
        calLink={calLink}
        style={{
          width: "100%",
          minHeight: "650px",
          // keep in DOM but hidden so iframe loads in background
          opacity: ready ? 1 : 0,
...
export default function BookPage() {
  const [selectedType, setSelectedType] = useState<BookingType["id"] | null>(
    null
  );
          ...
          <Card className="w-full overflow-hidden border-border/60">
            <CardHeader className="border-b border-border/60">
              ...
              <CardDescription>
                ...
                {selectedBooking.duration.toLowerCase()} appointments.
              </CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              {/* key prop forces full remount on type switch */}
              <CalEmbed
                key={selectedBooking.id}
                calLink={`swasthahomoeo/${selectedBooking.id}`}
              />
            </CardContent>
          </Card>
        )}
